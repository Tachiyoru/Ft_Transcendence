...
const AboutToPlay = () => {
  ...
  useEffect(() => {
    try {
      ...
      socket.on("findGame", (game) => {
        setGame(game);
        setLoading(false);
        if (!game) setError(true);
      });
      socket.on("bothReadyToStart", (data) => {
        if (data) playerReadyToStart++;
      ...
      setLoading(false);
      if (game) {
        socket.emit("verifyGame", {
          gameSocket: gameSocket,
          userId: userData?.id,
        });
        socket.on("verifyGame", (boolean) => {
          if (!boolean) setError(true);
        });
      }
    }
  }, [socket, userData]);
  ...
  useEffect(() => {
    ...
    socket.emit("setGoalsToWin", {
      gameSocket: gameSocket,
      goalsToWin: goalCount,
    });
        ...
        {game && !error ? (
          <div className="grid grid-cols-1 gap-20 text-center cursor-default">
            <div>
              ...
              <p className="font-audiowide text-1xl mb-1">
                First to [{game.goalsToWin}] goals wins (max 20)
              </p>

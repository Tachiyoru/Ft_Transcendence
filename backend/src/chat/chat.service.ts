...
export class chatService {
	...
	async getChannelsInCommon(userId: number, friendId: number)
	{
		...
		if (!userChannels || !friendChannels)
			return [];

		const channelsInCommon = userChannels.filter((channel) =>
			friendChannels.some((friendChannel) => friendChannel.chanId === channel.chanId)
		);
	...
	async renameChan(chanName: string, newName: string, @Request() req: any) {
		const chan = await this.prisma.channel.findUnique({
		where: { name: chanName },
		include: { owner: true },
		});
	...
	async acceptInvitationToChannel(chanName: string, @Request() req: any) {
		const chan = await this.prisma.channel.findUnique({
		where: { name: chanName },
		});
		if (!chan) {
	...
	async findAllMembers(chanName: string) {
		const chan = await this.prisma.channel.findUnique({
		where: { name: chanName },
	...
	async findAllBannedMembers(chanName: string) {
		const chan = await this.prisma.channel.findUnique({
		where: { name: chanName },
		include: { banned: true },
		});

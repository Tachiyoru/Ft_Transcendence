...
const SecurityEdit = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (userDataResponse.data.otpAuthUrl) setIsQrCode(true);
        if (userDataResponse.data.isTwoFaEnabled) setIsTwoFaEnabled(true);
      } catch (error) {
  ...
  return (
    <div className="mx-2">
      ...
      {/*CHANGE*/}
      {!(
        userData?.username.includes("_42") ||
        userData?.username.includes("_git")
      ) && (
        <form onSubmit={handleSubmit(submitHandler)}>
          ...
          <div className="mb-6 w-full">
            ...
            {errors.confirmPassword && (
              <div className="text-xs mt-2 text-red-orange">
        ...
        <div className="flex flex-row items-center mb-6">
          ...
          <label className="relative inline-flex items-center cursor-pointer">
            <input
              ...
              checked={isQrCode}
              onChange={() => handleQrCode(!isQrCode)}
            />
            <div
              className={`w-9 h-5 bg-dark-violet peer-focus:outline-none peer-focus:ring-0 rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full after:absolute after:top-[2px] after:start-[2px]  after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-purple ${
                isQrCode ? "after:bg-acid-green" : "after:bg-purple"
              }`}
            ></div>
            {isQrCode ? (
              <>
                <span className="absolute left-1 text-[0.45rem] text-lilac">
                  ON
                </span>
              </>
            ) : (
              <>
                <span className="absolute right-0.5 text-[0.45rem] text-lilac">
                  OFF
                </span>
              </>
            )}
